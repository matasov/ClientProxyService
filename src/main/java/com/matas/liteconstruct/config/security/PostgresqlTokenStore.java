...
@Slf4j
public class PostgresqlTokenStore implements TokenStore {
  ...
  private static final String DEFAULT_ACCESS_TOKEN_UPDATE_STATEMENT =
      "update oauth_access_token set token_id = ?, token = ? , authentication_id = ?, user_name = ?, client_id = ?, authentication = ?, refresh_token = ? where authentication_id = ?";
  ...
  private static final String EXTENDED_ACCESS_TOKEN_DELETE_STATEMENT =
      "delete from oauth_access_token where user_name = ? and client_id = ?";
  ...
  private String selectAccessTokensFromUserNameSql =
      DEFAULT_ACCESS_TOKENS_FROM_USERNAME_SELECT_STATEMENT;
  ...
  private String deleteAccessTokenFromRefreshTokenSql =
      DEFAULT_ACCESS_TOKEN_DELETE_FROM_REFRESH_TOKEN_STATEMENT;
  ...
  public PostgresqlTokenStore(DataSource dataSource) {
  ...
  public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
    ...
    try {
      ...
      log.error("find error for key: {}", key);
    } catch (org.springframework.dao.IncorrectResultSizeDataAccessException doubleEx) {
      log.error("too much values for: {}", key);
      return null;
    }
    return accessToken;
  }
  ...
  public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
    ...
    try {
      jdbcTemplate.update(insertAccessTokenSql, new Object[] {extractTokenKey(token.getValue()),
          new SqlLobValue(serializeAccessToken(token)), authId,
      ...
      log.error("yesss. update it!!! {}", dex);
      //
      jdbcTemplate.update(DEFAULT_ACCESS_TOKEN_UPDATE_STATEMENT,
          new Object[] {extractTokenKey(token.getValue()),
              new SqlLobValue(serializeAccessToken(token)), authId,
              authentication.isClientOnly() ? null : authentication.getName(),
              authentication.getOAuth2Request().getClientId(),
              new SqlLobValue(serializeAuthentication(authentication)),
              extractTokenKey(refreshToken), authId},
          new int[] {Types.VARCHAR, Types.BLOB, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
              Types.BLOB, Types.VARCHAR, Types.VARCHAR});
    }
  }
  ...
  public OAuth2AccessToken readAccessToken(String tokenValue) {
    ...
    try {
      accessToken =
          jdbcTemplate.queryForObject(selectAccessTokenSql, new RowMapper<OAuth2AccessToken>() {
            public OAuth2AccessToken mapRow(ResultSet rs, int rowNum) throws SQLException {
              return deserializeAccessToken(rs.getBytes(2));
            }
  ...
  public OAuth2RefreshToken readRefreshToken(String token) {
    ...
    log.info("readRefreshToken: {}", token);
    try {
      refreshToken =
  ...
  public void removeAllByUserAndService(String userName, String clientId) {
    jdbcTemplate.update(EXTENDED_ACCESS_TOKEN_DELETE_STATEMENT, userName, clientId);
  }
  ...
  public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
    ...
    try {
      accessTokens = jdbcTemplate.query(selectAccessTokensFromClientIdSql,
